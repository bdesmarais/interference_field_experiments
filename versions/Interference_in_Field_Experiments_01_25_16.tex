...
#### Transform uniformity trial outcome
#### into observed outcome
...
unif.to.z <- function(z, S, y.0, beta, tau){
  ...
  for (i in 1:n){
    spillover[i] <- beta +
	((1-z[i]) * (1-beta) * exp(-tau^2 * scalar[i]))
  }
...
#### Transform observed outcome
#### into uniformity trial outcome

z.to.unif <- function(z, S, y.0, beta, tau){
...
#### Transform observed outcome into
#### outcome for ANY other assignment w
...
z.to.w <- function(z, S, w, y.z, beta, tau){
  ...
  for (i in 1:n){
    spillover.z[i] <- beta +
	((1-z[i]) * (1-beta) * exp(-tau^2 * scalar.z[i]))
  }
  
  spillover.w <- rep(NA, n)
  for (i in 1:n){
    spillover.w[i] <- beta + ((1-w[i]) * (1-beta) * exp(-tau^2 * scalar.w[i]))
  }
  
  # Below is the actual function that
  # transforms observed outcomes into potential outcomes
  # Equation (6)
