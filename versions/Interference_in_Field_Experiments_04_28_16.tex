...
\section{For discussion on 04/06}
\begin{itemize}

\item Reviewing the poster
\item Reviewing the Coppock extension used in the poster
\item I would look forward to discussing the poster and the other idea of modeling nearest neighbors with you. I am happy to meet you on Skype. I really like the idea you have suggested for taking care of tied neighbors. I will try to code it in by tomorrow. After finishing the code that we now have, I was thinking of other ways in which we can further explore the idea of nearest neighbors. Maybe we could look at the number of nodes for which j is the nearest neighbor and in some way account for the j's influence on that basis? This does not fit into any of the 3 dimensions we have defined for extension. But it does touch upon an idea we had briefly discussed; that of looking at this model from the point of view of treated units instead of only control units. We can also turn it around in the following steps:
...
4. It seems intuitive to not model this parameter as linear, but will need to reason through it
...
\item {\bf Coppock method:} Coppock builds upon the New Mexico Legislator experiment conducted by Butler and Nickerson (2011). This paper also works with the idea of sharp null of no effects and uniformity trial. Using ideological similarity scores in the adjacency matrix, Coppock separates out the direct treatment effect and spillover occurring through the network.

In the first extension, we consider k nearest neighbors in terms of the ideology score. As of now, I have not taken care of ties. In 10 out of the 70 cases, there are in fact more than 3 neighbors within the same set of 3 distances.
\end{itemize}
