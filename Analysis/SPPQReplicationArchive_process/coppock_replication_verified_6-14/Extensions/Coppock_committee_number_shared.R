##################################################
#### Coppock extended with committees network ####
#### Edge is the number of committees together ####
##################################################
...
perms.test <- 50 #number of permutations used in testing

beta1s <- seq(from=-0.5, to=0.5, by=0.05)
beta2s <- seq(from=-0.5, to=0.5, by=0.05)
...
save(pvals, file="pvals_coppock_committee_2ormore.RData")
write.table(pvals, file="pvals_coppock_committee_2ormore.csv",
            col.names = beta2s, row.names = beta1s)


# Save
pdf("pval_plot_coppock_committee_2ormore.pdf")
image.plot(beta1s, beta2s, pvals,
           main = "Plot of p-values",
           xlab = "Direct effects", ylab = "Indirect effects")
