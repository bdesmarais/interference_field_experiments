#############################################
#### Coppock extended with knn adjacency ####
#############################################
...
setwd("~/Dropbox/Interference_in_Field_Experiments/Analysis/coppock_replication_data/Extensions") #SP
...
load("~/Dropbox/Interference_in_Field_Experiments/Analysis/coppock_replication_data/Original archival/CoppockJEPS.rdata") #SP
...
start.time <- Sys.time()
pvalue.nn.ideo <- foreach (i = 1:length(beta1s), .packages = "foreach") %dopar% {
  abc <- foreach (j = 1:length(beta2s)) %dopar% {
    ...
    results <- foreach (k = 1:perms.test) %dopar% {
      ...
      #perm.y.0 <- z.to.unif(outcome = y.z, beta1 = beta1s[i], beta2 = beta2s[j], permutation = z, adj.mat = nn.adj.mat)
...
for (i in 1:length(beta1s)){
  pvals[i,] <- unlist(pvalue.nn.ideo[i])
}
...
save(pvals, file="pvals_coppock_ideo_3nn.RData")
write.table(pvals, file="pvals_coppock_ideo_3nn.csv",
            col.names = beta2s, row.names = beta1s)


# Save
pdf("pval_plot_coppock_ideo_3nn.pdf")
image.plot(beta1s, beta2s, pvals,
...
pvalue.nn.ideo <- foreach (i = 1:length(beta1s)) %do% {
  abc <- foreach (j = 1:length(beta2s)) %do% {
    
    # Calculate the outcome vector after taking away the effect of treatment
    #y.0 <- z.to.unif(outcome = y.z, beta1 = beta1s[i], beta2 = beta2s[j], permutation = z, adj.mat = nn.adj.mat)
...
## Saving the p-value matrix
save(pvals, file="pvals_coppock_ideo_5nn.RData")
write.table(pvals, file="pvals_coppock_ideo_5nn.csv",
            col.names = beta2s, row.names = beta1s)


# Save
pdf("pval_plot_coppock_ideo_5nn.pdf")
image.plot(beta1s, beta2s, pvals,
...
## Saving the p-value matrix
save(pvals, file="pvals_coppock_ideo_8nn.RData")
write.table(pvals, file="pvals_coppock_ideo_8nn.csv",
            col.names = beta2s, row.names = beta1s)


# Save
pdf("pval_plot_coppock_ideo_8nn.pdf")
image.plot(beta1s, beta2s, pvals,
...
## Saving the p-value matrix
save(pvals, file="pvals_coppock_ideo_12nn.RData")
write.table(pvals, file="pvals_coppock_ideo_12nn.csv",
            col.names = beta2s, row.names = beta1s)
...
# To pull saved pvals and work with them
