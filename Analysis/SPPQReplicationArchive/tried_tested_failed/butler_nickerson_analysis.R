...
load("butler_cohort_copart_network.RData")
...
committee.net <- 1*(as.matrix(read.csv("butler_committee_number_shared.csv")[,-1])>0)
...
committee.net <- as.matrix(read.csv("butler_committee_number_shared.csv")[,-1])
copartisan <- matrix(0,nrow(data),nrow(data))
for(i in 1:nrow(data)){
  for(j in 1:nrow(data)){
    copartisan[i,j] <- 1*(data$party[i]==data$party[j])
