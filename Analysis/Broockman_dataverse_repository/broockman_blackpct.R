...
broockman.data <- broockman.data[is.na(broockman.data$blackpercent) == FALSE,]
...
# adj.mat <- as.matrix(read.csv("blackpct_network.csv"))

## Create the adjacency matrix using blackpercent
adj.mat <- NA
...
for (i in 1:length(unique(broockman.data$leg_state))){
  ...
  for (j in 1:length(broockman.data[broockman.data$leg_state==unique(broockman.data$leg_state)[i],1])){
    ...
    for (k in 1:length(broockman.data[broockman.data$leg_state==unique(broockman.data$leg_state)[i],1])){
      ...
      state.mat[j,k] <- get.similarity(broockman.data$blackpercent[j], broockman.data$blackpercent[k])
...
# write.csv(adj.mat, "blackpct_network.csv", row.names = FALSE, na = '')
...
beta1s <- seq(from=-0.3, to=0.3, by=.1)
beta2s <- seq(from=-0.3, to=0.3, by=.1)

pvals <- matrix(NA, length(beta1s), length(beta2s))

cl <- makeCluster(16) #Setup for parallel computing
...
save(pvals, file="pvals_broockman_blackpct.RData")
write.table(pvals, file="pvals_broockman_blackpct.csv",
            col.names = beta2s, row.names = beta1s)


# Save
pdf("pval_plot_broockman_blackpct.pdf")
image.plot(beta1s, beta2s, pvals,
           main = "Plot of p-values",
           xlab = "Direct effects", ylab = "Indirect effects")
