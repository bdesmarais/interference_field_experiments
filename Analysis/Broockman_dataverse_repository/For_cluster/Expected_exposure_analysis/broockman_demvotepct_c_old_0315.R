...
for (i in 1:length(beta1s)){
  abc <- foreach (j = 1:length(beta2s)) %do% {
    perm.test.stat <- rep(NA, perms.test)
    for (k in 1:perms.test){
      ...
      perm.test.stat[k] <- sum((lm(y.sim ~ perm.z + perm.exposure, na.action = na.omit)$resid)^2)
    }
    # Calculating p-value
    pvals[i,] <- sum(perm.test.stat < test.stat)/perms.test
  }
  print(i)
}
